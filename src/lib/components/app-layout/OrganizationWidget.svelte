...
{#snippet logoOrDefault(org, { className } = {})}
	...
	{#if sidePanel.isOpen}
		<span class={['hidden text-xs md:inline-block', !sidePanel.isOpen && 'md:!hidden']}
			>{org.name}</span>
	{/if}
{/snippet}

<li
	class="group -mx-6 mt-auto list-none md:grid md:grid-cols-6 md:hover:border-t md:hover:border-t-gray-400 duration-300">
	<a
		href="/dashboard/settings/organisation"
		class={[
			'hidden items-center gap-x-4 px-6 py-3 text-sm/6 font-semibold text-gray-900 group-hover:bg-gray-50 md:col-span-5 md:flex duration-300',
	...
	<TwDropdown
		...
		{#snippet customTrigger()}
			<button
				class={[
					'flex h-full w-full items-center justify-center pr-4 duration-300 md:border-l md:border-gray-300 md:pr-0 md:group-hover:border-gray-500 md:group-hover:bg-gray-300 md:hover:bg-gray-200',
				...
				{@render chevronRightIcon(
					`rotate-270 size-10 group-hover:text-gray-400 text-gray-300 hidden duration-300 ${!sidePanel.isOpen ? 'md:!hidden' : 'md:inline'}`
				)}
			</button>
		{/snippet}
	</TwDropdown>
