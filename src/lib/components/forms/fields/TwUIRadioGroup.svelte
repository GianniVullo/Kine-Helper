<script>
	let { group = $bindable(), label, description, outerCSS, options, error, ...rest } = $props();
</script>
...
<fieldset class={outerCSS}>
	...
	<div class={['mt-6 space-y-6 sm:flex sm:items-center sm:space-y-0 sm:space-x-10', rest.innerCSS]}>
		{#each options as { id: innerId, name, value, label }, index}
			...
			<div class="flex items-center">
				...
				<label for={id} class="ml-3 block text-sm/6 font-medium text-gray-900 dark:text-white"
					>{label}</label>
			</div>
		{/each}
	</div>
