<script>
	...
	import { Field, Form, FormSection } from './blocks/index';
	import WindowsSelectionField from './fields/WindowsSelectionField.svelte';
...
<Form id="peripheric-form" className="flex flex-col items-start w-full">
	<FormSection
		...
		{#await getPrinters then printers}
			{#if platform() === 'windows'}
				<WindowsSelectionField bind:printerField={formHandler.form['printer']} />
			{:else if platform() === 'macos'}
				<Field
					...
					bind:value={formHandler.form['printer']} />
			{/if}
		{/await}
		<Field
			field={{
