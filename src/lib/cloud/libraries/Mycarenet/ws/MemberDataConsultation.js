...
export class MemberDataConsultation extends SOAPService {
	...
	constructor({ onCreateEnveloppe, onResponse, certificate, pin, query }) {
		super({
			onCreateEnveloppe,
			onResponse,
			certificate,
			pin,
			samlToken: appState.eHealth.saml_token
		});
		this.query = query;
	}
	...
	async createEnveloppe() {
		const body = new BodyElement({
			...
			isTest: true,
			attribute_query_id: crypto.randomUUID(),
			query: this.query
		});
		const header = await this.wssecHeader({ hasBST: false, body });
		this.enveloppe = this.enveloppeWrapper(header.xmlString, body.canonicalized);
	}
}
...
export class BodyElement extends SecurityElement {
	...
	constructor({ request_uuid, issueInstant, isTest = true, attribute_query_id, query }) {
		...
		this.routingElement = new RoutingElement({ ssin: query.ssin, mut: query.mut_code });
...
class DetailElement {
	constructor({ attribute_query_id, query }) {
		//* <mcp:Detail>
		this.attributeQueryElement = new AttributeQueryElement({ attribute_query_id, query });
