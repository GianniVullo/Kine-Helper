<script>
	...
	let { attestationFormState, loading, codeMap } = createAttestationFormState(
		patient,
		untill,
		page
	);
		...
		async save() {
			...
			if (this.attestation.porte_prescr) {
				await dbAdapter.update(
					'prescriptions',
					[['prescription_id', attestation.prescription_id], 'user_id', appState.user.id],
					{
						...this.prescr,
						jointe_a
					}
	...
	async function isValid({ formData, submitter }) {
		console.log('in IsValid with', formData, attestationFormState);
		let codes = [];
		let codesNumber = 0;
		let totalValeurRecue = 0.0;
		let totalRemboursement = 0.0;
		for (let prescription of attestationFormState) {
	...
	function updateState() {
		updateAttestationFormState(patient, attestationFormState, page, $codeMap);
		state.update((s) => s);
	}
</script>
...
<FormWrapper {formSchema}>
	{#if $loading}
		{$t('shared', 'loading')}
	{:else}
		{#each attestationFormState as prescription}
